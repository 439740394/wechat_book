<template>
  <ebook-dialog :title="title" ref="dialog">
    <div class="dialog-list-wrapper" v-if="!ifNewGroup">
      <div class="dialog-list-item-wrapper"
...
import { removeToShelf, addToShelf } from '../../utils/store'
...
export default {
  ...
  methods: {
    ...
    createNewGroup () {
      ...
      const list = removeToShelf(this.shelfList)
      list.push(group)
      this.setShelfList(addToShelf(list))
      this.onComplete()
    },
    onComplete () {
